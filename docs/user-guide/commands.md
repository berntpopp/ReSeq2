...
```bash
reseq2 illuminaPE -j 32 -r ref.fa -b reads.bam -1 sim_R1.fq -2 sim_R2.fq
```
...
```bash
reseq2 seqToIllumina -j 2 -i sequences.fa -o reads.fq -s profile.reseq
```
...
```bash
reseq2 queryProfile -s profile.reseq --fragLenBias bias.tsv
```
...
```bash
reseq2 replaceN -r ref_with_ns.fa -R ref_clean.fa --seed 42
```
